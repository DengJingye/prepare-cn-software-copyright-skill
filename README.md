...
- 提醒手册/设计说明书必须包含结构图、流程图、运行截图或结果图，不能只有纯文字。
...
6. 为手册或设计说明书准备图片：系统结构图、运行流程图、终端运行截图、结果图或表格预览。
7. 最后按 `检查/软著材料检查清单.md` 检查文件名、版本号、页眉页脚、图片、源程序量和压缩包内容。
...
如果你本地有 Nature-style figure / scientific figure 相关 skill，可以让 Codex 用它生成结构图、流程图和结果图。终端截图建议使用不含中文路径、不暴露敏感信息的 demo 命令或测试命令。
